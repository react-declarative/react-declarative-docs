...
- [Installation](./installation.md) — Install react-declarative and its MUI peer dependencies in minutes.
- [Quick Start](./quickstart.md) — Build your first declarative form in under five minutes.
- [Field Types](./concepts/field-types.md) — Explore all 40+ field types: text, combo, date, rating, file, and more.
- [Component Reference](./api/components/one.md) — Full prop reference for One, List, Scaffold2, KanbanView, and WizardView.

## What you can build

react-declarative is not just a form library. It provides a complete toolkit for building data-driven React applications:

- [Forms & Validation](./components/one.md) — Nested grid forms with inline validation, conditional fields, and file uploads.
- [Data Grids](./components/list.md) — Filterable, sortable, paginated data grids with mobile-first adaptive layout.
- [App Shells](./components/scaffold.md) — Material Design app shells with navigation, tabs, and action menus from config.
- [Kanban Boards](./components/kanban.md) — Drag-and-drop kanban boards with real-time column updates.
